...
public class GameSimulation {
	...
	public void betweenTwo(){
		...
		if(chooseTeam==0){
			...
			this.actions.put(duration, "<p style= \"margin-bottom: 10px;text-align:center;margin-left: 5px;\">L'Èquipe "+game.getTeam1().getName()+" a gagnÈ l'entre deux</p>");
			teamHadBall= playersInGameTeam1;
			teamDontHadBall= playersInGameTeam2;
		}
		else {
			this.actions.put(duration, "<p style= \"margin-bottom: 10px;text-align:center;margin-left: 5px;\">L'Èquipe "+game.getTeam2().getName()+" a gagnÈ l'entre deux");
		...
		if(kindOfShot <= 60) {//2points
			...
			if(succesRate>lessThan) {
				...
				actions.put(duration,"<p  style= \"margin-bottom: 10px;margin-left: 5px;\" color=\"green\"><span style=\"font-size: 12px;font-weight:bold;\">" 
						...
						+ playerHadBall.getName() +" a mis un panier ‡† 2 points" + "</p>");
				int points=statsPoints.get(playerHadBall.getName())+2;
				statsPoints.put(playerHadBall.getName(), points);
			}
			else {
				actions.put(duration,"<p style= \"margin-bottom: 10px;margin-left: 5px;\" color=\"red\"><span style=\"font-size: 12px;font-weight:bold;\">"+teamDontBall.getName()+"</span> : " + interceptorPlayor.getName()+" a contrÈ un shot ‡† 2 point de "+ playerHadBall.getName()+ "</p> ");
			...
			if(succesRate>lessThan) {
				...
				actions.put(duration,"<p  style= \"margin-bottom: 10px;margin-left: 5px;\" color=\"green\"><span style=\"font-size: 12px;font-weight:bold;\">" 
						+ teamBall.getName() +"</span> ( "+game.getScore1()+"-"+game.getScore2()+" ) :"
						+ playerHadBall.getName() +" a mis un panier ‡† 3 points" + "</p>");
				int points=statsPoints.get(playerHadBall.getName())+3;
				statsPoints.put(playerHadBall.getName(), points);
				//System.out.println("shot 3pts");
			}
			else {
				actions.put(duration,"<p style= \"margin-bottom: 10px;margin-left: 5px;\" color=\"red\"><span style=\"font-size: 12px;font-weight:bold;\">"+teamDontBall.getName()+"</span> : " + interceptorPlayor.getName()+" a contrÈ un shot ‡† 3 point de "+ playerHadBall.getName()+ "</p> ");
				int blocks=statsBlocks.get(interceptorPlayor.getName())+1;
				statsBlocks.put(interceptorPlayor.getName(), blocks);
				//System.out.println("shot 3pts rat√©");
			}
		}
	...
	public void changment(int team){
		...
		if(team==0) {
			...
			actions.put(duration,"<p style= \"margin-bottom: 10px;margin-left: 5px;\" color=\"blue\"><span style=\"font-size: 12px;font-weight:bold;\">"+ game.getTeam1().getName() +"</span> : "+p.getName()+" est remplac√© par "+ playersInGameTeam1.get(positionToChange).getName()+ "</p>");
		}
		else {
			...
			actions.put(duration,"<p style= \"margin-bottom: 10px;margin-left: 5px;\" color=\"blue\"><span style=\"font-size: 12px;font-weight:bold;\">"+ game.getTeam2().getName() +"</span> : "+p.getName()+" est remplac√© par "+ playersInGameTeam2.get(positionToChange).getName()+ "</p>");
		}
		...
		if(!team.getPlayers().get(positionToChange-1).equals(playerIn)){
			...
			actions.put(duration,"<p color=\"blue\">"+players.get(positionToChange-1).getName()+" est remplac√© par "+ team.getPlayers().get(positionToChange-1).getName()+ "</p>");

		}
		else{
			players.add(team.getPlayers().get(positionToChange-1+5));
			actions.put(duration,"<p color=\"blue\">"+players.get(positionToChange-1).getName()+" est remplac√© par "+ team.getPlayers().get(positionToChange-1+5).getName()+ "</p>");
		}*/
	}
	...
	public boolean pass(ArrayList <Player> teamHadBall) {
		...
		float lessThan = MyRandom.getFloatIntoMinMax(0, 100);
		if(succesRate>lessThan) {
		//actions.put(duration, "<p>" + playerHadBall.getName() +" a pass√© la balle √† "+ destinatorPlayer.getName()+ "</p>");
			this.playerHadBall=destinatorPlayer;
			return true;
		}
		
		actions.put(duration, "<p style= \"margin-bottom: 10px;margin-left: 5px;\"><span style=\"font-size: 12px;font-weight:bold;\">"+teamDontBall.getName()+"</span> : " + interceptorPlayor.getName() +" a interceptÈ la balle ‡† "+ playerHadBall.getName()+ "</p> ");
	...
	public void teamShot(Team teamHadBall) {
		...
		if(kindOfShot <= 60) {//2points
			...
			if(succesRate>lessThan) {
				...
				//System.out.println("shot 2pts rat√©");
			}
