...
public class GameSimulation {
	...
	public void betweenTwo(){
		...
		if(chooseTeam==0){
			...
			this.actions.put(duration, "<p style= \"margin-bottom: 10px;text-align:center;\">L'Èquipe "+game.getTeam1().getName()+" a gagnÈ l'entre deux</p>");
			teamHadBall= playersInGameTeam1;
			teamDontHadBall= playersInGameTeam2;
		}
		else {
			this.actions.put(duration, "<p style= \"margin-bottom: 10px;text-align:center;\">L'Èquipe "+game.getTeam2().getName()+" a gagnÈ l'entre deux");
	...
	public void shot(ArrayList<Player> teamHadBall, Player playerHadBall) {
		...
		if(kindOfShot <= 60) {//2points
			...
			if(succesRate>lessThan) {
				...
				actions.put(duration,"<p  style= \"margin-bottom: 10px;\" color=\"green\">" + team.getName() +" : "+ playerHadBall.getName() +" a mis un panier ‡† 2 points" + "</p>");
				int points=statsPoints.get(playerHadBall.getName())+2;
				statsPoints.put(playerHadBall.getName(), points);
			}
			else {
				actions.put(duration,"<p style= \"margin-bottom: 10px;\" color=\"red\">" + interceptorPlayor.getName()+" a contrÈ un shot ‡† 2 point de "+ playerHadBall.getName()+ "</p> ");
			...
			if(succesRate>lessThan) {
				...
				actions.put(duration,"<p style= \"margin-bottom: 10px;\" color=\"green\">" + team.getName() +" : "+ playerHadBall.getName() +" a mis un panier ‡† 3 points" + "</p>");
				int points=statsPoints.get(playerHadBall.getName())+3;
				statsPoints.put(playerHadBall.getName(), points);
				//System.out.println("shot 3pts");
			}
			else {
				actions.put(duration,"<p style= \"margin-bottom: 10px;\" color=\"red\">" + interceptorPlayor.getName()+" a contrÈ© un shot ‡† 3 point de "+ playerHadBall.getName()+"</p>");
				int blocks=statsBlocks.get(interceptorPlayor.getName())+1;
				statsBlocks.put(interceptorPlayor.getName(), blocks);
	...
	public void changment(int team){
		...
		if(team==0) {
			...
			actions.put(duration,"<p style= \"margin-bottom: 10px;\" color=\"blue\">"+p.getName()+" est remplac√© par "+ playersInGameTeam1.get(positionToChange).getName()+ "</p>");
		}
		else {
			...
			actions.put(duration,"<p style= \"margin-bottom: 10px;\" color=\"blue\">"+p.getName()+" est remplac√© par "+ playersInGameTeam2.get(positionToChange).getName()+ "</p>");
		}
	...
	public boolean pass(ArrayList <Player> teamHadBall) {
		...
		actions.put(duration, "<p style= \"margin-bottom: 10px;\">" + interceptorPlayor.getName() +" a interceptÈ la balle √† "+ playerHadBall.getName()+ "</p> ");
